...
const updateCart = async (cart) => {
    ...
    const url = headless.cart_url.replace("{vendor}", headless.vendor).replace("{environment}", headless.environment);
...
const init = (configOverride) => {
    config = { ...config, ...configOverride }
    console.log("HC Init with [STORE]:" + config.store)
}

const start = async (cart) => {
    const updatedCart = await updateCart(cart)
    const hc_url = config.checkout_url.replace("{domain}", config.domain).replace("{cart_id}", updatedCart.token);
    console.log('HC URL is', hc_url);
    window.location.assign(hc_url);
    return updatedCart
}

const shopifyCartTransform = (cart) => {
...
const bigCommerceCartTransform = (cart) => cart

const transformers = {
    shopify: shopifyCartTransform,
    big_commerce: bigCommerceCartTransform
}

const HeadlessCheckout = { init, start, transformers}
